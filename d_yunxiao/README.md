...
- 缺陷：按创建时间从新到旧分页，按状态/负责人自动筛选（不选择即不限制）；列表和详情中的状态均为选择后立即修改并刷新列表；“待确认”“处理中”“再次打开”的整条缺陷卡片分别使用柔和的琥珀、蓝、玫红差异色，其余状态保持默认；支持查看描述/评论/附件，并以当前云效账号发布评论；详情展示创建人、最近修改人与更新时间，并可在详情内直接下拉更换负责人（候选来自项目成员列表，保存后同步刷新列表和最近修改人信息）；修改状态或负责人成功返回后约半秒再自动刷新列表，避开云效搜索索引的短暂延迟，避免列表闪回修改前的结果。
