...
- 缺陷通知：可按项目开启 Windows 系统通知、选择负责人和轮询间隔；自动解析并保存“待确认/再次打开”的状态 ID，每轮分别带两个状态条件查询一次（均按最后修改时间、第一页、最多 100 条），合并结果后统一按 ID 判断，并用每轮完整结果替换内存 ID 集合（空结果会重置为空，离开目标状态后再次进入可重新提醒）；Harness 桌面端优先使用其内置原生通知桥（与任务完成通知相同，无需浏览器授权弹窗），每次通知使用唯一标识并要求手动关闭，其他环境回退到 Windows 后端或 Web 通知；提供常驻内部通知弹窗、“测试内部通知”、“测试 Windows 通知”、“打开 Windows 通知设置”和“立即检查”，同时显示最近命中数、新增数、通知权限、实际通道与最近调用结果，并明确区分 Harness 桥授权与 Windows 横幅/勿扰设置；点击“查看缺陷”会切换到缺陷页并打开第一条新增缺陷的详情。
