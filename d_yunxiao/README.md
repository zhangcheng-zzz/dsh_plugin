...
- 缺陷：按创建时间从新到旧分页，按状态/负责人自动筛选（不选择即不限制）；列表和详情中的状态均为选择后立即修改并刷新列表；“待确认”“处理中”“再次打开”的整条缺陷卡片分别使用柔和的琥珀、蓝、玫红差异色，其余状态保持默认；支持查看描述/评论/附件，并以当前云效账号发布评论；详情展示创建人、最近修改人与更新时间，并可在详情内直接下拉更换负责人（候选来自项目成员列表，保存后同步刷新列表和最近修改人信息）。
- 缺陷通知：可按项目开启 Windows 系统通知、选择负责人和轮询间隔；自动解析并保存“待确认/再次打开”的状态 ID，每轮分别带两个状态条件查询一次（均按最后修改时间、第一页、最多 100 条），合并结果后统一按 ID 判断，并用每轮完整结果替换内存 ID 集合（空结果会重置为空，离开目标状态后再次进入可重新提醒）；Harness 桌面端同时提交内置原生通知桥与桌面右下角的 Windows 非模态提醒（无需浏览器授权弹窗），桌面提醒不依赖通知中心并在 15 秒后自动关闭；若窗体初始化失败则退化为明确可见的系统消息框；其他环境回退到 Windows 后端或 Web 通知；提供常驻内部通知弹窗、“测试内部通知”、“测试 Windows 通知”、“打开 Windows 通知设置”和“立即检查”，同时显示最近命中数、新增数、通知权限、实际通道与最近调用结果；点击“查看缺陷”会切换到缺陷页并打开第一条新增缺陷的详情。
